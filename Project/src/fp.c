...
void parse_configuration(char * cmd_data, uint16_t cmd_len)
{
    char idx = 0;
    ExpBoardParametersDI48_t *par = (ExpBoardParametersDI48_t *) cmd_data;

    if (sizeof(ExpBoardParametersDI48_t) != par->header.size) {
	//printf("global-cfg: bad size! Wait: %d != Read: %d\n", sizeof(ExpBoardParametersDI48_t), cmd_len);
	//*ack_len = 0;
        return;
    }

    //printf("Получена новая конфигурация, применяем\n");

    if (par->common.poll_period >= MIN_EXCHANGE_PERIOD)
        exchange_period = par->common.poll_period;
    
    par->common.leds_cfg_state ?
				leds_all_on() :
				leds_all_off();

    leds_set_timeout(par->common.leds_timeout);
    led_on(LED_USER, par->common.user_led == 0 ? LED_OFF :
                     par->common.user_led == 1 ? LED_RED : LED_GREEN, 0);

   
    for (idx=0; idx<PORTS_COUNT; idx++)
    {
	port_queue_set_state	(idx, par->port[idx].pm_state ?	CH_STATE_ON	: CH_STATE_OFF);
	port_queue_set_mode	(idx, SwitchToBoardType(par->port[idx].mode));

	//Эти функции тоже отложенные, так как не приминяют изменения сразу.
        //alarm_set_port_value	(idx, par->port[idx].alarm_value);
	port_set_block_state	(idx, par->port[idx].bs_pm_state ? CH_STATE_ON	: CH_STATE_OFF, SwitchToBoardType(par->port[idx].mode));
    }
  /*
    */
}
		...
		uint8_t *ack_data, uint16_t *ack_len)
{
	...
	case CMD_PORT_SET_BLOCK_STATE        : {
		uint8_t num = *(uint8_t *)cmd_data;
		uint8_t state = *(uint8_t *)(cmd_data+sizeof(uint8_t));
		uint8_t mode = *(uint8_t *)(cmd_data+2*sizeof(uint8_t));
		port_set_block_state(num, (enum ch_state)state, (enum ch_mode)mode);
		*ack_len = 0;
		break;	
	}
	case CMD_PORT_GET_BLOCK_STATE        : {
		uint8_t num = *(uint8_t *)cmd_data;
		enum ch_state state = CH_STATE_OFF;
		enum ch_mode mode = CH_MODE_DI_DC_2MA;
		port_get_block_state(num, &state, &mode);
		*(uint8_t*)ack_data = num;
		*(uint8_t *)(ack_data+sizeof(uint8_t)) = state;
		*(uint8_t *)(ack_data+2*sizeof(uint8_t)) = mode;
		*ack_len = 3;
		break;	
	}
	case CMD_PORT_APPLY_BLOCK_STATE      : {
	...
	case CMD_PORT_GET_STATUS              : {
		uint8_t num = *(uint8_t *)cmd_data;
		enum mod_status status = STATUS_NORMAL;
		port_get_status(num, &status);
		*(uint8_t*)ack_data = num;
		*(uint8_t *)(ack_data+sizeof(uint8_t)) = status;
		*ack_len = 2;
		break;	
	}
	...
	case CMD_LEDS_GET_STATE               : {
		*(uint8_t*)ack_data = leds_get_is_all_on();
		*ack_len = 1;
		break;	
	}
	case CMD_LEDS_SET_TIMEOUT           : {
		uint8_t timeout = *(uint8_t *)cmd_data;
		leds_set_timeout(timeout);
		*ack_len = 0;
		break;	
	}
	case CMD_LEDS_GET_TIMEOUT           : {
		*(uint8_t*)ack_data = leds_get_timeout();
		*ack_len = 1;
		break;	
	}

	case CMD_USER_LED_SET           	: {
		enum led_state type = *(uint8_t *)cmd_data;
		led_on(LED_USER, type, 0);
		*ack_len = 0;
		break;	
	}
	case CMD_USER_LED_GET           	: {
		*(uint8_t*)ack_data = leds_get_user_led();
		*ack_len = 1;
		break;	
	}
	...
	case CMD_SET_BLOCK_STATE              : {
		uint8_t state = *(uint8_t *)(cmd_data);
		block_state_set_int(state);
		*ack_len = 0;
		break;	
	}
	case CMD_GET_BLOCK_STATE              : {
		*(uint8_t*)ack_data = block_state_get_ext();
		*(uint8_t *)(ack_data+sizeof(uint8_t)) = block_state_get_int();
		*ack_len = 2;
		break;	
	}
	...
	case CMD_RWIO : { 				
		...
		switch (data_type) {
			case PHYSICAL_VALUE: {
				...
				pd[1] = CHANNELNUMBER_48;	
				asm("CPSID i");
					memcpy(&pd[4], board_set.values.value, sizeof(unsigned char) * PORTS_COUNT);				
				asm("CPSIE i");

				*ack_len = 4 + sizeof(unsigned char) * PORTS_COUNT;									
				break;
			}
			
			case INCREMENTAL_TEST: 
			{
				static volatile uint16_t incrementing_value = 0;
				uint16_t* pa = ((uint16_t *)(ack_data+4));
				int i;
				
				if (++incrementing_value >= 0x100) incrementing_value = 0;
				for (i = 0; i<PORTS_COUNT; i++) {
					pa[i] = incrementing_value | (i<<12);
				}
				*ack_len = 4 + PORTS_COUNT * sizeof(unsigned char);
				break;		
			}
			
			default: //Ошибка, ничего отвечать не будем. 
				*ack_len = 0;	
				break;	
		}			
		break; 
